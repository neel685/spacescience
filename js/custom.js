...
/* globals window, document, navigator, Custom_ENV_TEST */

/* eslint-disable no-var */

/**
 * Custom: Core site library
 */

var Custom = {};
var modules = {};
var primary = [];
var secondary = window.Custom || [];
var $ = window.jQuery;
var $win = $(window);
var $doc = $(document);
var isFunction = $.isFunction;

var _ = Custom._ = __webpack_require__(132);

var tram = Custom.tram = __webpack_require__(68) && $.tram;
var domready = false;
var destroyed = false;
tram.config.hideBackface = false;
tram.config.keepInherited = true;
/**
 * Custom.define - Define a named module
 * @param  {string} name
...
Custom.define = function (name, factory, options) {
  if (modules[name]) {
...
/**
 * Custom.require - Require a named module
 * @param  {string} name
 * @return {object}
 */


Custom.require = function (name) {
  return modules[name];
};

function bindModule(module) {
  // If running in Custom app, subscribe to design/preview events
  if (Custom.env()) {
...
/**
 * Custom.push - Add a ready handler into secondary queue
 * @param {function} ready  Callback to invoke on domready
 */


Custom.push = function (ready) {
...
/**
 * Custom.env - Get the state of the Custom app
 * @param {string} mode [optional]
 * @return {boolean}
 */


Custom.env = function (mode) {
...
var touch = Custom.env.touch = 'ontouchstart' in window || window.DocumentTouch && document instanceof window.DocumentTouch;
var chrome = Custom.env.chrome = /chrome/.test(userAgent) && /Google/.test(navigator.vendor) && parseInt(userAgent.match(/chrome\/(\d+)\./)[1], 10);
var ios = Custom.env.ios = /(ipod|iphone|ipad)/.test(userAgent);
Custom.env.safari = /safari/.test(userAgent) && !chrome && !ios; // Maintain current touch target to prevent late clicks on touch devices
...
/**
 * Custom.validClick - validate click target against current touch target
 * @param  {HTMLElement} clickTarget  Element being clicked
 * @return {Boolean}  True if click target is valid (always true on non-touch)
 */

Custom.validClick = touch ? function (clickTarget) {
  return clickTarget === touchTarget || $.contains(clickTarget, touchTarget);
} : function () {
  return true;
};
/**
 * Custom.resize, Custom.scroll - throttled event proxies
 */

var resizeEvents = 'resize.Custom orientationchange.Custom load.Custom';
var scrollEvents = 'scroll.Custom ' + resizeEvents;
Custom.resize = eventProxy($win, resizeEvents);
Custom.scroll = eventProxy($win, scrollEvents);
Custom.redraw = eventProxy(); // Create a proxy instance for throttled events
...
function eventProxy(target, types) {
  ...
  return proxy;
} // Custom.location - Wrap window.location in api


Custom.location = function (url) {
  window.location = url;
};

if (Custom.env()) {
  // Ignore redirects inside a Custom design/edit environment
  Custom.location = function () {};
} // Custom.ready - Call primary and secondary handlers
...
/**
 * Custom.load - Add a window load handler that will run even if load event has already happened
 * @param  {function} handler
 */
...
} // Custom.destroy - Trigger a destroy event for all modules


Custom.destroy = function (options) {
  options = options || {};
  destroyed = true;
  ...
  Custom.resize.off();
  Custom.scroll.off();
  Custom.redraw.off(); // Clear any queued ready methods
...
$(Custom.ready); // Listen for window.onload and resolve deferred

bindLoad(); // Export commonjs module

module.exports = window.Custom = Custom;
...
/**
 * Custom: IX Event triggers for other modules
 */
...
var _IX2BrowserSupport = __webpack_require__(47); // eslint-disable-next-line Custom/module-top-level-imports, Custom/packages-must-be-defined
...
/**
 * Custom: Brand pages on the subdomain
 */

var Custom = __webpack_require__(6);

Custom.define('brand', module.exports = function ($) {
  var api = {};
  var doc = document;
  var $html = $('html');
  var $body = $('body');
  var namespace = '.w-Custom-badge';
  ...
  api.ready = function () {
    ...
    if (/\.Custom\.io$/i.test(publishedDomain) && location.hostname !== publishedDomain) {
      shouldBrand = true;
    }
  ...
  function createBadge() {
    var $brand = $('<a class="w-Custom-badge"></a>').attr('href', 'https://Custom.com?utm_campaign=brandjs');
    var $logoArt = $('<img>').attr('src', 'https://d3e54v103j8qbb.cloudfront.net/img/Custom-badge-icon.f67cd735e3.svg').attr('alt', '').css({
      marginRight: '8px',
      width: '16px'
    });
    var $logoText = $('<img>').attr('src', 'https://d1otoma47x30pg.cloudfront.net/img/Custom-badge-text.6faa6a38cd.svg').attr('alt', 'Made in Custom');
  ...
  function ensureBrand() {
    ...
    var inEditor = Custom.env('editor');
...
/*!
 * Custom._ (aka) Underscore.js 1.6.0 (custom build)
 * _.each
 * _.map
 * _.find
 * _.filter
...
module.exports = function () {
  ...
  _.VERSION = '1.6.0-Custom'; // Establish the object that gets returned to break out of a loop iteration.
...
/* globals document, window, localStorage, Custom_API_HOST, Custom_DEFAULT_HOST */

/* eslint-disable no-var */

/**
 * Custom: Editor loader
 */

var Custom = __webpack_require__(6);

Custom.define('edit', module.exports = function ($, _, options) {
  options = options || {}; // Exit early in test env or when inside an iframe

  if (Custom.env('test') || Custom.env('frame')) {
  ...
  try {
    ...
    hasLocalStorage = localStorage && localStorage.getItem && localStorage.getItem('CustomEditor');
  ...
  function load() {
    loaded = true; // Predefine global immediately to benefit Custom.env

    window.CustomEditor = true;
    $win.off(hashchange, checkHash);
    checkThirdPartyCookieSupport(function (thirdPartyCookiesSupported) {
      $.ajax({
        url: cleanSlashes("https://editor-api.Custom.com" + '/api/editor/view'),
        data: {
  ...
  function prefix(url) {
    return url.indexOf('//') >= 0 ? url : cleanSlashes("https://editor-api.Custom.com" + url);
  }
  ...
  function checkThirdPartyCookieSupport(callback) {
    ...
    iframe.src = "https://Custom.com" + '/site/third-party-cookie-check.html';
...
Custom.define('focus-visible', module.exports = function () {
  /**
...
ix2.setEnv(Custom.env);
Custom.define('ix2', module.exports = function () {
  return ix2;
});
...
var createPluginInstance = function createPluginInstance(element) {
  var instance = window.Custom.require('lottie').createInstance(element);
...
/**
 * Custom: Auto-select links to current page or section
 */

var Custom = __webpack_require__(6);

Custom.define('links', module.exports = function ($, _) {
  var api = {};
  var $win = $(window);
  var designer;
  var inApp = Custom.env();
  var location = window.location;
  ...
  function init() {
    designer = inApp && Custom.env('design');
    slug = Custom.env('slug') || location.pathname || ''; // Reset scroll listener, init anchors

    Custom.scroll.off(scroll);
  ...
  function select(link) {
    ...
    if (tempLink.hash.length > 1 && tempLink.host + tempLink.pathname === location.host + location.pathname) {
      ...
      // https://forum.Custom.com/t/dropdown-menus-not-working-on-site/87140
...
Custom.define('scroll', module.exports = function ($) {
  /**
  ...
  var rootTag = Custom.env('editor') ? '.w-editor-body' : 'body';
  ...
  function validateScroll(evt) {
    ...
    Custom.env('design') || // Ignore links being used by jQuery mobile
  ...
  function updateHistory(hash) {
    ...
    !(Custom.env.chrome && loc.protocol === 'file:')) {
...
Custom.define('touch', module.exports = function ($) {
  var api = {};
...
/* globals
  ...
  document,
  FormData,
  Custom_FORM_API_HOST,
  Custom_FORM_OLDIE_HOST
*/
...
Custom.define('forms', module.exports = function ($, _) {
  var api = {};
  var $doc = $(document);
  var $forms;
  var loc = window.location;
  ...
  function init() {
    siteId = $('html').attr('data-wf-site');
    formUrl = "https://Custom.com" + '/api/v1/form/' + siteId; // Work around same-protocol IE XDR limitation - without this IE9 and below forms won't submit

    if (retro && formUrl.indexOf("https://Custom.com") >= 0) {
      formUrl = formUrl.replace("https://Custom.com", "http://formdata.Custom.com");
    }
  ...
  function build(i, el) {
    ...
    if (action) {
      return;
    } // Custom forms for hosting accounts


    if (siteId) {
      data.handler = typeof hostedSubmitCustom === 'function' ? hostedSubmitCustom : exportedSubmitCustom;
      return;
    } // Alert for disconnected Custom forms
  ...
  function addListeners() {
    listening = true; // Handle form submission for Custom forms
  ...
  function exportedSubmitCustom(data) {
    preventDefault(data);
    afterSubmit(data);
  }
  /* Custom_HOSTED_BLOCK:START */
  // Submit form to Custom


  function hostedSubmitCustom(data) {
    reset(data);
    var form = data.form;
    var payload = {
...
Custom.define('navbar', module.exports = function ($, _) {
  var api = {};
  var tram = $.tram;
  var $win = $(window);
  var $doc = $(document);
  ...
  function removeListeners() {
    Custom.resize.off(resizeAll);
  }

  function addListeners() {
    Custom.resize.on(resizeAll);
  }
  ...
  function navigate(data) {
    return function (evt) {
      ...
      if (!Custom.validClick(evt.currentTarget)) {
        evt.preventDefault();
        return;
  ...
  function open(data, immediate) {
    ...
    resize(0, navbarEl);
    ix.intro(0, navbarEl);
    Custom.redraw.up(); // Listen for click outside events
...
/******/ ]);/**
 * ----------------------------------------------------------------------
 * Custom: Interactions 2.0: Init
 */
Custom.require('ix2').init(
{"events":{"e":{"id":"e","animationType":"preset","eventTypeId":"SCROLL_INTO_VIEW","action":{"id":"","actionTypeId":"SLIDE_EFFECT","instant":false,"config":{"actionListId":"slideInBottom","autoStopEventId":"e-2"}},"mediaQueries":["main","medium","small","tiny"],"target":{"id":"61de4a43cff440ca3bc02116|e464d218-f801-55d1-1f50-7da00b5bfb8f","appliesTo":"ELEMENT","styleBlockIds":[]},"targets":[{"id":"61de4a43cff440ca3bc02116|e464d218-f801-55d1-1f50-7da00b5bfb8f","appliesTo":"ELEMENT","styleBlockIds":[]}],"config":{"loop":false,"playInReverse":false,"scrollOffsetValue":20,"scrollOffsetUnit":"%","delay":0,"direction":"BOTTOM","effectIn":true},"createdOn":1582925126668}},"actionLists":{"slideInBottom":{"id":"slideInBottom","useFirstGroupAsInitialState":true,"actionItemGroups":[{"actionItems":[{"actionTypeId":"STYLE_OPACITY","config":{"delay":0,"duration":0,"target":{"id":"N/A","appliesTo":"TRIGGER_ELEMENT","useEventTarget":true},"value":0}}]},{"actionItems":[{"actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"duration":0,"target":{"id":"N/A","appliesTo":"TRIGGER_ELEMENT","useEventTarget":true},"xValue":0,"yValue":100,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}}]},{"actionItems":[{"actionTypeId":"TRANSFORM_MOVE","config":{"delay":0,"easing":"outQuart","duration":1000,"target":{"id":"N/A","appliesTo":"TRIGGER_ELEMENT","useEventTarget":true},"xValue":0,"yValue":0,"xUnit":"PX","yUnit":"PX","zUnit":"PX"}},{"actionTypeId":"STYLE_OPACITY","config":{"delay":0,"easing":"outQuart","duration":1000,"target":{"id":"N/A","appliesTo":"TRIGGER_ELEMENT","useEventTarget":true},"value":1}}]}]}},"site":{"mediaQueries":[{"key":"main","min":992,"max":10000},{"key":"medium","min":768,"max":991},{"key":"small","min":480,"max":767},{"key":"tiny","min":0,"max":479}]}}
);
